...
global x0 y0 theta0 phi0% condizioni iniziali
global x y theta phi u u1vett u2vett
global termine1 termine2 termine3 termine4 termine5 termine6
...
for i=1:N
    ...
    theta(i+1)=theta(i) + Tc*( u(i)*(tan(phi(i))/L) );
    phi(i+1)=phi(i) + Tc*( u(N+i) );
end
...
phiobiettivoGrad = 0; % cambiare valore (gradi) per variare orientazione finale
phiobiettivo = deg2rad(thetaobiettivoGrad); % conversione in radianti
termine6=K3*sum((phi-(phiobiettivo)).^2);

Costo=Tc*(termine1+termine2+termine3+termine4+termine5+termine6);
